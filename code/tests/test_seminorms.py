...
def solveit(atom, Z, W, U, linq, L, FISTA):
        ...
        yield ac, Z-atom.proximal(1, Z, 0), d.proximal(1, Z, 0), 'testing dual of projections starting from atom %s ' % atom
        ...
        # yield ac, atom.proximal(L, Z, 0), solver.composite.coefs, 'solving prox with simple_problem.nonsmooth with monotonicity %s ' % atom
        ...
        yield ac, atom.proximal(L, Z, 0), solver.composite.coefs, 'solving prox with simple_problem with monotonicity %s ' % atom
        ...
        # yield ac, atom.proximal(L, Z, 0), solver.composite.coefs, 'solving prox with simple_problem.nonsmooth with no monotonocity %s ' % atom
        ...
        yield ac, atom.proximal(L, Z, 0), solver.composite.coefs, 'solving prox with simple_problem %s no monotonicity_restart' % atom
        ...
        yield ac, atom.proximal(L, Z, 0), solver.composite.coefs, 'solving atom prox with separable_atom.singleton %s ' % atom
        ...
        yield ac, atom.proximal(L, Z, 0), solver.composite.coefs, 'solving atom prox with container %s ' % atom

        loss = rr.quadratic.shift(-Z, coef=0.5*L)
        problem = rr.simple_problem(loss, d)
        solver = rr.FISTA(problem)
        solver.fit(tol=1.0e-12, monotonicity_restart=False, FISTA=FISTA)
        yield ac, d.proximal(L, Z, 0), solver.composite.coefs, 'solving dual prox with simple_problem no monotonocity %s ' % atom

        problem = rr.container(d, loss)
        solver = rr.FISTA(problem)
        solver.fit(tol=1.0e-12, FISTA=FISTA)
        yield ac, d.proximal(L, Z, 0), solver.composite.coefs, 'solving dual prox with container %s ' % atom
        ...
        yield ac, d.proximal(L, Z, 0), solver.composite.coefs, 'solving atom prox with separable_atom.singleton %s ' % atom
