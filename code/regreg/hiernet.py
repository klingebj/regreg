...
class heirnet(lasso):
    ...
    def lagrange_max(self):
        if not hasattr(self, "_lagrange_max"):
            ...
            if self.intercept:
                self._lagrange_max = self.loss.smooth_objective(null_soln, 'grad')[1:].max()
            else:
                self._lagrange_max = self.loss.smooth_objective(null_soln, 'grad').max()
    ...
    def strong_set(self, lagrange_cur, lagrange_new, 
        ...
        value += s.adjoint_map(s.linear_map(grad) < (slope_estimate+1) * lagrange_new - slope_estimate*lagrange_cur)
        return ~value
    ...
    def check_KKT(self, tol=1.0e-02):
        ...
        failing += s.adjoint_map(g_s > lagrange * (1 + tol))

        # Check the active coefficients
        active = soln_s != 0
        failing_s[active] += (g_s[active] / lagrange + 1) >= tol 
