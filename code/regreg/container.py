...
class container(object):
    ...
    def dual_prox(self, u, lipschitz_D=1.):
        ...
        for atom, dual_atom, segment in zip(self.atoms, self.dual_atoms, self.dual_segments):
            if atom.constraint:
                v[segment] = dual_atom.primal_prox(u[segment], lipschitz_D)
            else:
                v[segment] = atom.dual_prox(u[segment], lipschitz_D)
    ...
    def primal_prox(self, y, lipschitz_P=1, with_history=False, debug=False, max_its=5000, tol=1e-14):
        ...
        if not hasattr(self, 'dualopt'):
            self.dualp = self.dual_problem(yL, lipschitz_P=lipschitz_P)
        ...
        self.dualopt.problem.smooth_multiplier = 1./lipschitz_P
        ...
        if with_history:
            return self.primal_from_dual(y, self.dualopt.problem.coefs/lipschitz_P,
                                         tol=tol), history
        else:
            return self.primal_from_dual(y, self.dualopt.problem.coefs/lipschitz_P)
    ...
    def dual_problem(self, y, lipschitz_P=1, initial=None):
        """
        ...
        prox = self.dual_prox
        return dummy_problem(self._dual_smooth_eval, nonsmooth, prox, initial, 1./lipschitz_P)
